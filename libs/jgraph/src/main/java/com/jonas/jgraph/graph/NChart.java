...
public class NChart extends View implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {
    ...
    private Paint mCurvePaint;//TODO 曲线画笔
    /**
     * 图表 画笔
     */
     ...
     * 柱状图 选中的颜色
     */
    private int mNormalColor = Color.parseColor("#4EB5D7");
    /**
     * 横坐标 信息颜色
     */
    ...
    private void init(Context context) {
        ...
        mCurvePaint.setPathEffect(new CornerPathEffect(20));//圆角
        mCurvePaint.setColor(getNormalColor());

//        mCurvePaint.setShader(new LinearGradient(0, 20, 0, mHeight, Color.RED, Color.GREEN, Shader.TileMode.CLAMP));
        //拿不到  height的值
...
//        mLinePointRadio = dip2px(4);
    ...
    private boolean isInterceptAnimate;

    /**
     * 是否拦截动画
     *
     * @param interceptAnimate
     */
    public void setInterceptAnimate(boolean interceptAnimate) {
        isInterceptAnimate = interceptAnimate;
    }
...
//        mHeightRatio = (mHCoordinate - 2 * mTextSize - mAbove - mTextMarging - mLinePointRadio - 5) / mSugHeightest;
        ...
        mWidth = w;
        if (mExcels.size() > 0 && !isInterceptAnimate) {
            animateExcels();
        }
...
//        drawCoordinateAxes(canvas);
    ...
    private void drawSugExcel_BAR(Canvas canvas) {
        for (int i = 0; i < mExcels.size(); i++) {
            ...
            float right = 0;
            float bottom = 0;
            float diameter = right - left;

            //画柱子 + 动画
            if (mBarAniStyle == ChartAniStyle.BAR_UP) {
                left = excel.getRectF().left;
                top = excel.getRectF().bottom - (excel.getHeight()) * ratio;
                right = excel.getRectF().right;
                bottom = excel.getRectF().bottom;
            } else if (mBarAniStyle == ChartAniStyle.BAR_RIGHT) {
                //左往右 增大
                left = excel.getRectF().left;
                top = excel.getRectF().top;
                right = excel.getRectF().left + (excel.getWidth()) * ratio;
                bottom = excel.getRectF().bottom;
                ...
                top = excel.getRectF().top;
                right = excel.getRectF().right;
                bottom = excel.getRectF().top + (excel.getHeight()) * ratio;
            } else {
                ...
                //以mBarStanded为准 左右散开
                left = sExcel.getRectF().left + (excel.getRectF().left - sExcel.getRectF().left) * ratio;
                top = excel.getRectF().top;
                right = sExcel.getRectF().right + (excel.getRectF().right - sExcel.getRectF().right) * ratio;
                bottom = excel.getRectF().bottom;
            }

            diameter = right - left;
            RectF rectTop = new RectF(left, top, right, top + diameter);
            canvas.drawArc(rectTop, 0, -180, true, mExecelPaint);
            RectF rectF = new RectF(left, top - 1 + diameter / 2, right, bottom);
            canvas.drawRect(rectF, mExecelPaint);
...
//        textBg.close();
    ...
    private boolean isInterceptDrawText;
     ...
     * @param intercept
     */
    public void setInterceptDrawText(boolean intercept) {
        this.isInterceptDrawText = intercept;
    }
...
//            canvas.drawPoint(midPointF.x, midPointF.y, mLinePaint);
    ...
    private void drawSugExcel_CloseLINE(Canvas canvas) {
        ...
        float minY = ys.get(0);
        float maxY = ys.get(ys.size() - 1);

        PointF firstP = mExcels.get(0).getMidPointF();
        pathLine.moveTo(firstP.x, firstP.y);
        int size = mExcels.size();
        for (int i = 0; i < size; i++) {
            ...
            if (midPointF.y == minY || midPointF.y == maxY) {
                ...
                int index = 0;
                    ...
                    index = i + 1;
                boolean istop = excel.getMidPointF().y < mExcels.get(index).getMidPointF().y;
                if (i == 0 || i == size - 1)
                    istop = true;//
                String text = mFormat.format(excel.getUpper()) + excel.getUnit() + textUnit;
                float x = midPointF.x;
                float y = istop ? (midPointF.y - textMarging) : (midPointF.y + mTextSize + mLinePointRadio) * ratio;
                float y2 = istop ? (excel.getRectF().bottom - (excel.getHeight() + textMarging) * ratio) : (excel.getRectF().bottom - (excel.getHeight() - mTextSize - mLinePointRadio) * ratio);
                mTextPaint.setColor(getNormalColor());
                if (mTextAniStyle == TextAniStyle.ANIDOWN) {//向下飘
                    canvas.drawText(text, x, y, mTextPaint);
                } else {//文字向上飘
                    canvas.drawText(text, x, y2, mTextPaint);
                }
            }
        }
        canvas.drawPath(pathLine, mCurvePaint);
    }
...
//        canvas.drawLine(0, mHCoordinate, mWidth, mHCoordinate, mExecelPaint);
     ...
     * @param color
     */
    public void setTextPrintColor(int color) {
        mTextColor = color;
    }
     ...
     * 设置绘制文本的单位
     */
    public void setDrawTextUnit(String unit) {
        textUnit = unit;
    }
